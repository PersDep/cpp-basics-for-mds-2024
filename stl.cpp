...
struct list_node {
  int value; // any type
  list_node *next;
  list_node *prev; // this makes the list double linked
};

int main(void)
{
    std::list<int> my_list; // see the way it is constructed on line 49

    // Create a vector of integers.
    std::vector<int> my_vector; // Talk about templates in C++
    // int_64t, int_32t, int_16t, int_8t, uint_64t, uint_32t, uint_16t, uint_8t
    ...
    std::cout << "Current values in cycle:" << std::endl;
    for (auto element : my_vector) {
        element *= 3;
        std::cout << element << std::endl;
    }

    std::cout << "Values in container after cycle 1:" << std::endl;
    ...
    std::cout << "Values in container after cycle 2:" << std::endl;
    for (std::vector<int>::iterator it = my_vector.begin(); it < my_vector.end(); ++it) {
        std::cout << *it << std::endl;
    }
    ...
    int a = 10;
    int *b = new int[10]; // new -- C++ operator
    // calloc(), malloc() - C style alocation
    // if you use new -> use delete for memory deallocation
    // if you use calloc()/malloc() -> use free() for memory deallocation
    // Out Of Memory error
    ...
    *(b + 2) = 21;
    ...
    std::map<std::string, int> my_plain_map; // uses binary search
    // for the key type you need to have comparison (<, >, ==) implemented
    // Always O(log n) search / insert
    std::unordered_map<std::string, int> my_map; // uses hash map for search: O(1)
    // for the key type you need to have hash implemented
    // O(n) collisions on some insertions
  ...
  // <algorithms>
  // std::sort(my_vector_to_sort.begin(), my_vector_to_sort.end(), [](int a, int b) { return a > b; });
