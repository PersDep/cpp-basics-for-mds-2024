...
int main(void)
{
    ...
    //assembler: data sections; stack: block of memory which has faster access and less vloume

    long long a0 = 15000000000; // up to 10 gigabytes on heap or even more until you have enough RAM
    // const int a1 = 10000000; // we can get 1 megabyte on stack, but 10 megabytes are already causing the crash
    // float d;
    
    {
      std::unique_ptr<char> a2(new char[a0]); // -> right way to implement
    }

    {
      char *e = new char[a0]; // raw pointer // calloc, malloc
   ...
   // 1. you should use objects for allocation of arrays/lists/maps/vectors
   // 2. if you need pointers, use smart pointers
